...
fn do_test<'a, T: game_system::System<'a>>(font_file_content: &'a [u8]) -> Result<(), String> {
    ...
    {
        let mut test_text =
            system.text("hello world".try_into()?, NonZero::new(127).unwrap(), None)?;
    ...
    //     if let Some(game_system::Event::Quit) = maybe_event {
    //         break;
    //     }
    //     println!("{} {:?}", (Instant::now() - before).as_millis(), maybe_event);
