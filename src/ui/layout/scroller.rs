...
pub enum ScrollerSizingPolicy {
    ...
    Custom(CustomSizing, ScrollAspectRatioDirectionPolicy),
}
...
pub struct Scroller<'state, 'a, T: crate::core::System<'a>> {
    /// for drag scrolling
...
impl<'state, 'a, T: crate::core::System<'a>> Scroller<'state, 'a, T> {
    pub fn new(
        scroll_x_enabled: bool,
        scroll_y_enabled: bool,
        scroll_x: &'state Cell<i32>,
        scroll_y: &'state Cell<i32>,
        contains: Box<dyn Widget<'a, T>>,
    ) -> Self {
        Self {
...
fn apply_scroll_restrictions(
    mut position_for_contained: crate::ui::util::rect::FRect,
    event_position: crate::ui::util::rect::FRect,
    scroll_y: &mut i32,
    scroll_x: &mut i32,
) {
...
impl<'state, 'a, T: crate::core::System<'a>> Widget<'a, T> for Scroller<'state, 'a, T> {
    fn min(
        &mut self,
    ) -> Result<
        (
    ...
    fn update(&mut self, mut event: WidgetUpdateEvent) -> Result<(), String> {
        ...
        let clip_rect_for_contained = self
            .previous_clipping_rect_from_update
            .intersection(self.position_from_update.into());
    ...
    fn draw(
        ...
        debug_assert!(sys_interface.clip_rect() == self.previous_clipping_rect_from_update);
        sys_interface.set_clip_rect(
            self.previous_clipping_rect_from_update
                .intersection(self.position_from_update.into()),
        );
        let draw_result = self.contained.draw(sys_interface);
        sys_interface.set_clip_rect(self.previous_clipping_rect_from_update); // restore
        draw_result
    }
}
