...
pub trait System<'font_data>: Sized {
    ...
    fn texture(&mut self, image_path: &Path) -> Result<impl Texture<'_>, String>;

    /// render text or reuse from (unspecified) cache. if needed, prepares the
    /// the font for the given point size. the texture instance can then be used
    ...
    ///
    /// the returned texture will do some unspecified pixel interpolation /
    /// smoothing when scaled
    fn text(
        &mut self,
        text: NonEmptyStr,
        point_size: NonZeroU16,
        wrap_width: Option<NonZeroU32>,
    ) -> Result<impl Texture, String>;
